<nav x-data="{ open: false }" class="bg-white border-b border-slate-100 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-slate-500 bg-white hover:text-slate-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::user()->name }} <span class="text-xs text-teal-600 font-bold">({{ strtoupper(Auth::user()->role) }})</span></div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-xl text-slate-400 hover:text-slate-500 hover:bg-slate-50 focus:outline-none transition duration-150 ease-in-out">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-slate-50/60 border-t border-slate-100">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(in_array(auth()->user()->role, ['kaprodi', 'tu', 'manager']))
                <x-responsive-nav-link :href="route('dashboard')" :active="false">
                    {{ __('Daftar Mahasiswa (' . (auth()->user()->department->name ?? 'Umum') . ')') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link :href="route('dashboard')" :active="false">
                    {{ __('Daftar Staf Dosen & TU (' . (auth()->user()->department->name ?? 'Umum') . ')') }}
                </x-responsive-nav-link>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-slate-200">
            <div class="px-4">
                <div class="font-bold text-base text-slate-800">{{ Auth::user()->name }} <span class="text-xs text-teal-600 font-bold">({{ strtoupper(Auth::user()->role) }})</span></div>
